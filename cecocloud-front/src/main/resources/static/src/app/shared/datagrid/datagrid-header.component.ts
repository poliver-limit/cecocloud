...
export class DatagridHeaderComponent implements IHeaderGroupAngularComp {
	...
	agInit(params: any): void {
		...
		this.selectionSubscription = params.api['getDatagridComponent']().selectionSubject.subscribe(event => {
			this.anyRowSelected = event.api.getSelectedRows().length > 0;
		});
		// Actualitza la informacio de paginació
		this.paginationEnabled = params.api['getDatagridComponent']().gridOptions.paginationEnabled;
		this.paginationSubscription = params.api['getDatagridComponent']().paginationSubject.subscribe(event => {
			this.paginationEnabled = params.api['getDatagridComponent']().gridOptions.paginationEnabled;
		...
		this.paginationSubscription = params.api['getDatagridComponent']().scrollSubject.subscribe(event => {
			this.calculateScrollFirstAndLastRow();
		});
	}
	...
	onButtonRefreshClick() {
		this.params.api['getDatagridComponent']().refreshInternal();
	}
	onButtonCreateClick() {
		this.params.api['getDatagridComponent']().headerActionCreate.emit({});
	}
	onButtonImportClick() {
		console.log('>>> onButtonImportClick')
	}
	onButtonDeleteClick() {
		this.params.api['getDatagridComponent']().headerActionDelete.emit({
			resource: this.restapiProfile.resource,
			selectedRows: this.params.api.getSelectedRows()
		});
	}

	onQuickFilterKeypress(event: Event) {
	...
	calculateScrollFirstAndLastRow() {
		...
		let rowHeight = this.params.api['getDatagridComponent']().rowHeight;
