...
@Component({
    ...
    <bng-datagrid
        ...
        [restapiService]="identificadorUsuarisService"
...
export class IdentificadorUsuarisGridComponent {
    ...
    onGridActionCreate() {
        ...
        const dialogRef = this.dialog.open(IdentificadorUsuarisAddDialog, {
            width: '500px'
        });
        dialogRef.afterClosed().subscribe(usuari => {
            if (usuari) {
                ...
                this.identificadorUsuarisService.create(usuariIdentificador).subscribe(() => {
        ...
        if (event.selectedRows) {
            if (event.selectedRows.length == 1) {
                ...
                if (confirm(confirmMessageTranslated)) {
                    this.identificadorUsuarisService.deleteById(event.selectedRows[0].id).subscribe(() => {
                        this.datagrid.refresh();
                    });
                }
            } else {
                ...
                if (confirm(confirmMessageTranslated)) {
                    ...
                    this.identificadorUsuarisService.deleteBulk(ids).subscribe(() => {
                        this.datagrid.refresh();
                    });
                }
            }
    ...
    constructor(
        ...
        public identificadorUsuarisService: IdentificadorUsuarisService) {
    }
...
@Component({
    template: `
<h1 mat-dialog-title>{{'page.identificador-usuaris.modal.titol' | translate}}</h1>
...
<div mat-dialog-actions style="justify-content:space-between">
	<button mat-raised-button (click)="onCancelButtonClick()">{{'page.identificador-usuaris.modal.button.cancelar' | translate}}</button>
	<button mat-raised-button color="primary" [mat-dialog-close]="usuari" [disabled]="!usuari">{{'page.identificador-usuaris.modal.button.afegir' | translate}}</button>
</div>
`,
})
export class IdentificadorUsuarisAddDialog implements AfterViewInit {
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<IdentificadorUsuarisAddDialog>,
