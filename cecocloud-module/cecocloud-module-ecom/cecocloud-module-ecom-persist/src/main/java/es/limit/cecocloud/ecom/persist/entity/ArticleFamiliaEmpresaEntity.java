...
@Entity(name = "ecomArticleFamiliaEmpresaEntity")
@Table(
		name = "tcom_fae",
		indexes = {
...
@AssociationOverrides({
	@AssociationOverride(
			...
			foreignKey = @ForeignKey(name = "rcom_fae_idf_fk"))
})
public class ArticleFamiliaEmpresaEntity extends AbstractWithIdentificadorAuditableEntity<ArticleFamiliaEmpresa, ArticleFamiliaEmpresaPk> {
	...
	@JoinColumns(
			...
			foreignKey = @ForeignKey(name = "rcom_fae_far_fk"))			
	...
	@JoinColumns(
				...
				foreignKey = @ForeignKey(name = "rcom_fae_emp_fk"))
