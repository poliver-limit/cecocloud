...
import es.limit.cecocloud.ecom.persist.entity.DocumentPagamentCobramentEntity.DocumentPagamentCobramentEntityListener;
...
@Entity(name = "ecomDocumentPagamentCobramentEntity")
@Table(
		name = "tcom_dpg",
		indexes = {
				@Index(name = "icom_dpg_idf_fk", columnList = "dpg_idf_cod"),
				@Index(name = "ircom_dpg_pk", columnList = "dpg_idf_cod,dpg_cod", unique = true)
		}
)
@AttributeOverrides({
...
@AssociationOverrides({
	@AssociationOverride(
			...
			foreignKey = @ForeignKey(name = "rcom_dpg_idf_fk"))
})
@EntityListeners({DocumentPagamentCobramentEntityListener.class})
public class DocumentPagamentCobramentEntity extends AbstractWithIdentificadorAuditableEntity<DocumentPagamentCobrament, WithIdentificadorAndCodiPk<String>> {
	...
	@JoinColumns(
			...
			foreignKey = @ForeignKey(name = "rcom_dpg_npg_fk"))
	...
	@JoinColumns(
			...
			foreignKey = @ForeignKey(name = "rcom_dpg_iva_fk"))
	...
	@JoinColumns(
			...
			foreignKey = @ForeignKey(name = "rcom_dpg_rgi_fk"))
	...
	public static class DocumentPagamentCobramentEntityListener {
		@PrePersist
		public void calcular(DocumentPagamentCobramentEntity documentPagamentCobrament) {
			String codi = documentPagamentCobrament.getEmbedded().getCodi();
			if (codi == null || codi.isEmpty()) {
				int seq = EntityListenerUtil.getSeguentNumComptadorComprovantPk(
						documentPagamentCobrament.getId().getIdentificadorCodi(),
						"TCOM_DPG",
						new PkBuilder<WithIdentificadorAndCodiPk<String>>() {
							@Override
							public WithIdentificadorAndCodiPk<String> build(int seq) {
								return new WithIdentificadorAndCodiPk<String>(documentPagamentCobrament.getId().getIdentificadorCodi(), Integer.toString(seq));
							}
						},
						EntityListenerUtil.getBean(DocumentPagamentCobramentRepository.class));
				String seqST = addZeros(seq, 4);
				documentPagamentCobrament.getEmbedded().setCodi(seqST);
				documentPagamentCobrament.getId().setCodi(seqST);
			} else {
				if (isNumeric(codi)) {					
					codi = addZeros(Integer.parseInt(codi), 4);
					documentPagamentCobrament.getEmbedded().setCodi(codi);
					documentPagamentCobrament.getId().setCodi(codi);
				}
			}
		}
	}
