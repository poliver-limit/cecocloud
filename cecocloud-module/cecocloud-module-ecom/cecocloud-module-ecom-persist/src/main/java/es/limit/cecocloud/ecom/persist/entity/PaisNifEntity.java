...
@Entity(name = "ecomPaisNifEntity")
@Table(
		name = "tcom_pni",
		uniqueConstraints = {
		@UniqueConstraint(name = "rcom_pni_pk", columnNames = "pni_cod")
		}
)
@AttributeOverrides({
	@AttributeOverride(name = "id", column = @Column(name = "pni_cod", length = 2)),
...
public class PaisNifEntity extends AbstractSimpleEntity<PaisNif, String>{
	...
	@Formula(value="(SELECT CONCAT(CONCAT(pni.pni_nom,' - '),pni.pni_cod) FROM tcom_pni pni where pni.pni_cod = pni_cod)")
