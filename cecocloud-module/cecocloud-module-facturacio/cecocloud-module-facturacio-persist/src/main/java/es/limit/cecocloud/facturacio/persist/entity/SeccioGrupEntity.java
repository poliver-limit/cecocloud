...
@Table(
		name = "tges_gse",
		indexes = {
				@Index(name = "iges_gse_idf_fk", columnList = "gse_idf_cod"),
				@Index(name = "irges_gse_pk", columnList = "gse_idf_cod,gse_cod", unique = true)
		}
)
@AttributeOverrides({
...
public class SeccioGrupEntity extends AbstractAuditableCompositePkEntity<SeccioGrup, SeccioGrupPk> {
	...
	@JoinColumn(
			...
			insertable = false,
			updatable = false,
			foreignKey = @ForeignKey(name = "rges_gse_idf_fk"))
