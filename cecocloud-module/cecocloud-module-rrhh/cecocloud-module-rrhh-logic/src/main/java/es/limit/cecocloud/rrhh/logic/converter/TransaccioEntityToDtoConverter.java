/**
 * 
 */
package es.limit.cecocloud.rrhh.logic.converter;

import org.springframework.stereotype.Component;

import es.limit.base.boot.logic.converter.AbstractEntityToDtoConverter;
import es.limit.cecocloud.rrhh.logic.api.dto.Transaccio;
import es.limit.cecocloud.rrhh.persist.entity.TransaccioEntity;

/**
 * Conversor cap a DTO de les entitats de tipus Transaccio.
 * 
 * @author Limit Tecnologies <limit@limit.es>
 */
@Component
public class TransaccioEntityToDtoConverter extends AbstractEntityToDtoConverter<TransaccioEntity, Transaccio> {

}