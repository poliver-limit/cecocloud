/**
 * 
 */
package es.limit.cecocloud.rrhh.logic.converter;

import org.springframework.stereotype.Component;

import es.limit.base.boot.logic.converter.AbstractEntityToDtoConverter;
import es.limit.cecocloud.rrhh.logic.api.dto.GrupFestiu;
import es.limit.cecocloud.rrhh.persist.entity.GrupFestiuEntity;

/**
 * Conversor cap a DTO de les entitats de tipus GrupFestiu.
 * 
 * @author Limit Tecnologies <limit@limit.es>
 */
@Component
public class GrupFestiuEntityToDtoConverter extends AbstractEntityToDtoConverter<GrupFestiuEntity, GrupFestiu> {

}