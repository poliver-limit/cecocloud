...
@AttributeOverrides({
	...
	@AttributeOverride(name = "embedded.numHoresLaborals", column = @Column(name = "reg_maxhorlab", precision=3, scale=0)),
	@AttributeOverride(name = "embedded.numMinHoresLaborals", column = @Column(name = "reg_minhorlab", precision=3, scale=0)),
...
@AssociationOverrides({
	@AssociationOverride(
			name = "identificador",
			joinColumns = {
					@JoinColumn(name = "reg_idf_cod", insertable = false, updatable = false)
			},
			foreignKey = @ForeignKey(name = "rrhu_reg_idf_fk"))
})
public class RegimEntity extends AbstractWithIdentificadorAuditableEntity<Regim, WithIdentificadorAndCodiPk<String>> {
