...
@Table(
		name = "tges_cxa",
		indexes = {
				@Index(name = "irges_cxa_pk", columnList = "cxa_idf_cod,cxa_emp_cod,cxa_cod", unique = true),
				@Index(name = "iges_cxa_idf_fk", columnList = "cxa_idf_cod"),
				@Index(name = "iges_cxa_emp_fk", columnList = "cxa_emp_cod")
		}
)
@AttributeOverrides({
...
@AssociationOverrides({
	@AssociationOverride(
			...
			foreignKey = @ForeignKey(name = "rges_cxa_idf_fk"))
})
public class CaixaEntity extends AbstractWithIdentificadorAuditableEntity<Caixa, CaixaPk> {
	...
	@JoinColumns(
			...
			foreignKey = @ForeignKey(name = "rges_cxa_emp_fk"))
