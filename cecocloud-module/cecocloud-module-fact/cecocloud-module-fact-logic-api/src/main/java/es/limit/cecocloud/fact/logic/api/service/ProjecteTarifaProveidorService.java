/**
 * 
 */
package es.limit.cecocloud.fact.logic.api.service;

import es.limit.base.boot.logic.api.service.GenericCompositePkService;
import es.limit.cecocloud.fact.logic.api.dto.ProjecteTarifaProveidor;

/**
 * Servei per a la gestió de ProjecteTarifaProveidor.
 * 
 * @author Limit Tecnologies <limit@limit.es>
 */
public interface ProjecteTarifaProveidorService extends GenericCompositePkService<ProjecteTarifaProveidor> {

}
