package es.limit.cecocloud.facturacio.logic.converter;

import org.springframework.stereotype.Component;

import es.limit.base.boot.logic.converter.AbstractEntityToDtoConverter;
import es.limit.cecocloud.facturacio.logic.api.dto.Rappel;
import es.limit.cecocloud.facturacio.persist.entity.RappelEntity;

/**
 * Conversor cap a DTO de les entitats de tipus de rappel.
 * 
 * @author Limit Tecnologies <limit@limit.es>
 */
@Component
public class RappelEntityToDtoConverter extends AbstractEntityToDtoConverter<RappelEntity, Rappel> {

}
